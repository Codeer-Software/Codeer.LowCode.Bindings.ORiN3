...
namespace Codeer.LowCode.Bindings.ORiN3.Server
{
    internal class O3Setting
    {
        ...
        internal record ORiN3RootObjectSetting(Guid Id, string ProviderId, string Version, string Host, string Name,
            ...
            bool UseRemoteEngineTelemetrySetting, string LogLevel, int Protocol, bool SpecifyCertificate, string CertificateFile,
            string CertificatePassword, IList<TelemetryEndpoint> TelemetryEndpoints, IList<Tuple<string, string>> Extension);
        internal record TelemetryEndpoint(string Uri, string ProxyAddress, int ProxySetting, int TelemetryTypeFlag, int ProtocolType);
        ...
        public O3Setting(JsonNode json)
        {
            ...
            foreach (var root in json!["RootObjects"]!.AsArray())
            {
                ...
                var logLevel = root!["LogLevel"]!.GetValue<string>();
                ...
                foreach (var endpoint in root!["TelemetryEndpoints"]!.AsArray())
                {
                    ...
                    var proxySetting = endpoint!["ProxySetting"]!.GetValue<int>();
                    var telemetryTypeFlag = endpoint!["TelemetryTypeFlag"]!.GetValue<int>();
                    var protocolType = endpoint!["ProtocolType"]!.GetValue<int>();
                    telemetryEndpoints.Add(new(uri, proxyAddress, proxySetting, telemetryTypeFlag, protocolType));
                }
                var extension = new List<Tuple<string, string>>();
                foreach (var it in root!["Extension"]!.AsArray())
                {
                    var key = it!["Key"]!.GetValue<string>();
                    var value = it!["Value"]!.GetValue<string>();
                    extension.Add(new(key, value));
                }
